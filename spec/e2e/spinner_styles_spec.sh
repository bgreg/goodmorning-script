...
# CRITICAL: Test that fetch_with_spinner actually captures command output
# Note: Uses external commands because ShellSpec's It-block functions
# aren't visible to background subshells created by fetch_with_spinner
It 'captures command output correctly'
result=$(fetch_with_spinner "Testing..." echo "test_output_12345" 2>/dev/null)
The value "$result" should equal "test_output_12345"
End

It 'captures multiline output'
result=$(fetch_with_spinner "Testing..." printf '%s\n%s\n' "line1" "line2" 2>/dev/null)
The value "$result" should include "line1"
The value "$result" should include "line2"
End

It 'handles command with JSON output'
result=$(fetch_with_spinner "Testing..." echo '{"key":"value"}' 2>/dev/null)
The value "$result" should equal '{"key":"value"}'
End

# Note: Testing failing commands is tricky with spinners
# The core functionality (capturing output) is what matters most
End
...
It 'all styles have non-empty character arrays'
check_all_chars_non_empty() {
  local errors=0
  ...
  for style_name in "${SPINNER_STYLE_NAMES[@]}"; do
    ...
    if [ "${#chars_array[@]}" -eq 0 ]; then
      echo "EMPTY CHARS: $style_name" >&2
      errors=$((errors + 1))
    fi
  done
...
When call check_all_chars_non_empty
The status should be success
End

It 'all styles have numeric backspace entries'
check_all_backspaces_numeric() {
  local errors=0

  for style_name in "${SPINNER_STYLE_NAMES[@]}"; do
    local backspaces="${SPINNER_STYLES_BACKSPACES[$style_name]}"

    if ! [[ "$backspaces" =~ ^[0-9]+$ ]]; then
      echo "INVALID BACKSPACES: $style_name = '$backspaces'" >&2
      errors=$((errors + 1))
    fi
  done
...
When call check_all_backspaces_numeric
The status should be success
End

It 'SPINNER_STYLE_NAMES count matches SPINNER_STYLES_CHARS entries'
check_names_match_chars() {
  local names_count="${#SPINNER_STYLE_NAMES[@]}"
  local chars_count="${#SPINNER_STYLES_CHARS[@]}"

  [ "$names_count" -eq "$chars_count" ]
}
When call check_names_match_chars
The status should be success
End

It 'SPINNER_STYLE_NAMES count matches SPINNER_STYLES_BACKSPACES entries'
check_names_match_backspaces() {
  local names_count="${#SPINNER_STYLE_NAMES[@]}"
  local backspaces_count="${#SPINNER_STYLES_BACKSPACES[@]}"

  [ "$names_count" -eq "$backspaces_count" ]
}
When call check_names_match_backspaces
The status should be success
End
End

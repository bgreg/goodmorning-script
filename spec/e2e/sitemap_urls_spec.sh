...
setup() {
  ...
  LEARNING_SOURCES="$project_root/data/learning-sources.json"
  SANITY_SOURCES="$project_root/data/sanity-maintenance-sources.json"
  RESULTS_DIR=$(mktemp -d)
  TIMEOUT_SECONDS=10
  if [ -z "${SHELLSPEC_REAL:-}" ]; then
    PATH="${project_root}/spec/support/bin:${PATH}"
    export PATH
  fi
}
...
has_sitemaps() {
  local count
  count=$(jq -r '.sitemaps | length' "$LEARNING_SOURCES")
  [ "$count" -gt 0 ]
}
When call has_sitemaps
The status should be success
End
...
has_static() {
  local count
  count=$(jq -r '.static | length' "$LEARNING_SOURCES")
  [ "$count" -gt 0 ]
}
When call has_static
The status should be success
End
...
has_categories() {
  local count
  count=$(jq -r '.categories | keys | length' "$SANITY_SOURCES")
  [ "$count" -gt 0 ]
}
When call has_categories
The status should be success
End
...
check_https_sanity() {
  local non_https
  non_https=$(jq -r '.categories | to_entries[] | .value[] | .url | select(startswith("https://") | not) | select(startswith("xkcd:") | not)' "$SANITY_SOURCES")
  if [ -n "$non_https" ]; then
    echo "Invalid URL protocol: $non_https"
    return 1
  fi
  return 0
}

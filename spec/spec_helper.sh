...
source_goodmorning() {
  ...
  export GOODMORNING_NO_AUTO_RUN=1
  # Always use test config directory to prevent loading user's personal config
  # This ensures tests are isolated from user settings
  export GOODMORNING_CONFIG_DIR="/tmp/goodmorning-test-$$"
  mkdir -p "$GOODMORNING_CONFIG_DIR/logs" 2>/dev/null

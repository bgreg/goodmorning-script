...
Describe 'setup.sh - Input Validation'
  ...
  Describe 'Doctor validation functions'
    It 'has terminal features check'
      When call grep 'doctor_check_terminal_features' "$PROJECT_ROOT/lib/setup/doctor.sh"
      The status should be success
      The output should not be blank
    End

    It 'checks for iTerm2 support'
      When call grep 'TERM_PROGRAM.*iTerm' "$PROJECT_ROOT/lib/setup/doctor.sh"
      The status should be success
      The output should not be blank
    End

    It 'has validation pass function'
      When call grep 'validation_pass' "$PROJECT_ROOT/lib/setup/validation_helpers.sh"
      The status should be success
      The output should not be blank
    End

    It 'has validation fail function'
      When call grep 'validation_fail' "$PROJECT_ROOT/lib/setup/validation_helpers.sh"
      The status should be success
      The output should not be blank
    End
  End

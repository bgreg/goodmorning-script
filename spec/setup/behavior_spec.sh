...
Describe 'Variable Pollution Prevention'
# Regression test: sourcing library files should not overwrite caller's SCRIPT_DIR
# Bug: validation_helpers.sh was overwriting SCRIPT_DIR, breaking setup.sh paths

It 'sourcing validation_helpers.sh preserves existing SCRIPT_DIR'
test_script_dir_preserved() {
  local original_script_dir="/test/original/path"
  SCRIPT_DIR="$original_script_dir"
  source ./lib/setup/validation_helpers.sh
  [ "$SCRIPT_DIR" = "$original_script_dir" ]
}
When call test_script_dir_preserved
The status should be success
End

It 'sourcing validation.sh preserves existing SCRIPT_DIR'
test_validation_preserves() {
  local original_script_dir="/test/original/path"
  SCRIPT_DIR="$original_script_dir"
  source ./lib/validation.sh
  [ "$SCRIPT_DIR" = "$original_script_dir" ]
}
When call test_validation_preserves
The status should be success
End

It 'setup.sh sources do not overwrite SCRIPT_DIR'
# Simulates what setup.sh does: set SCRIPT_DIR then source libs
test_setup_sourcing() {
  local project_root="$(pwd)"
  SCRIPT_DIR="$project_root"
  source "$SCRIPT_DIR/lib/app/colors.sh"
  source "$SCRIPT_DIR/lib/validation.sh"
  # SCRIPT_DIR should still point to project root, not lib/
  [ "$SCRIPT_DIR" = "$project_root" ]
}
When call test_setup_sourcing
The status should be success
End
End
End

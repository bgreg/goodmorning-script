...
Describe 'lib/app/updates.sh - Background Updates'
  ...
  Describe 'macOS notification'
    It 'uses correct AppleScript path for notifications'
      When call grep 'lib/app/apple_scripts/show_notification.scpt' "$PROJECT_ROOT/lib/app/updates.sh"
      The status should be success
      The output should not be blank
    End
  ...
  Describe '_update_homebrew function'
    ...
    It 'runs brew cleanup'
      When call grep 'brew cleanup' "$PROJECT_ROOT/lib/app/updates.sh"
      The status should be success
      The output should not be blank
    End
  End
  ...
  Describe '_update_vim_plugins function'
    It 'is defined'
      When call type _update_vim_plugins
      The status should be success
      The output should include "function"
    End

    It 'uses GOODMORNING_VIM_PLUGINS_DIR variable'
      When call grep 'GOODMORNING_VIM_PLUGINS_DIR' "$PROJECT_ROOT/lib/app/updates.sh"
      The status should be success
      The output should not be blank
    End

    It 'checks for git directories'
      When call grep '\.git' "$PROJECT_ROOT/lib/app/updates.sh"
      The status should be success
      The output should not be blank
    End

    It 'runs git pull for each plugin'
      When call grep 'git pull' "$PROJECT_ROOT/lib/app/updates.sh"
      The status should be success
      The output should not be blank
    End
  End

  Describe '_update_nvim_plugins function'
    It 'is defined'
      When call type _update_nvim_plugins
      The status should be success
      The output should include "function"
    End

    It 'checks for lazy.nvim plugin directory'
      When call grep 'nvim/lazy' "$PROJECT_ROOT/lib/app/updates.sh"
      The status should be success
      The output should not be blank
    End
  End

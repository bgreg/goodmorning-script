...
prompt_for_variable() {
  ...
  echo ""
  echo_yellow "Setting up ${var_name}"
  echo_blue "  $description"

  local display_default="${current_value:-$default_value}"

  while true; do
    if [ -n "$display_default" ]; then
      echo_green -n "Enter value [$display_default]: "
    else
      echo_green -n "Enter value: "
    fi
    read -r user_input
...
run_interactive_setup() {
  ...
  echo_cyan "Press Enter to skip any optional setting."
  echo ""
  ...
  user_name=$(prompt_for_variable \
    ...
    "Your name for the morning greeting" \
    "$USER" \
    "$current_user_name")

  echo ""
  echo_yellow "Text-to-Speech Greeting"
  echo_blue "Enable spoken 'Good morning' greeting? (requires 'say' command)"
  if [ "$current_enable_tts" = "true" ]; then
    echo_blue "Current: enabled"
  else
    echo_blue "Current: disabled (default)"
  fi
  ...
  backup_script=$(prompt_for_variable \
    ...
    "Path to backup script (optional)" \
    "" \
  ...
  vim_plugins_dir=$(prompt_for_variable \
    ...
    "Vim plugins directory (optional)" \
    "$HOME/.vim/pack/vendor/start" \
    "$current_vim_plugins_dir" \
    "validate_directory_path")

  echo ""
  echo_yellow "Setting up GOODMORNING_PROJECT_DIRS"
  echo_blue "Description: Colon-separated directories to scan for git repos"
  echo_blue "Default: $HOME"
  if [ -n "$current_project_dirs" ]; then
    echo_blue "Current: $current_project_dirs"
  else
    echo_blue "Current: (none)"
  fi
  echo_blue "Example: $HOME/workspace:$HOME/projects"
  echo_green -n "Enter directories (space or colon-separated, press Enter to skip): "
  ...
  if [ -n "$user_name" ]; then
    echo ""
    echo_yellow "ASCII Art Banner Setup"
    echo_blue "Would you like to generate a custom ASCII art banner for your name?"
    echo_blue "(This requires figlet to be installed)"
  ...
  # Copy learning sources file
  if [ -f "$SCRIPT_DIR/learning-sources.txt" ]; then
    if [ ! -f "$CONFIG_DIR/learning-sources.txt" ]; then
      cp "$SCRIPT_DIR/learning-sources.txt" "$CONFIG_DIR/learning-sources.txt"
      print_success "Copied learning-sources.txt to config directory"
    fi
  fi
...
show_config() {
  ...
  echo_cyan "Configuration directory: $CONFIG_DIR"
  echo_cyan "Configuration file: $CONFIG_FILE"
  echo ""

  source "$CONFIG_FILE"

  echo_yellow "GOODMORNING_USER_NAME:"
  echo "  ${GOODMORNING_USER_NAME:-$USER}"
  echo ""

  echo_yellow "GOODMORNING_BACKUP_SCRIPT:"
  ...
  echo_yellow "GOODMORNING_VIM_PLUGINS_DIR:"
  ...
  echo_yellow "GOODMORNING_PROJECT_DIRS:"

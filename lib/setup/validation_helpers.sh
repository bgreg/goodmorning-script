...
# Determine the lib/ directory path for local sourcing (don't overwrite global SCRIPT_DIR)
_VALIDATION_LIB_DIR="${0:a:h:h}"

if [[ -z "$COLOR_RESET" ]]; then
  source "$_VALIDATION_LIB_DIR/app/colors.sh" 2>/dev/null || true
fi

if [ -f "$_VALIDATION_LIB_DIR/app/view_helpers.sh" ]; then
  source "$_VALIDATION_LIB_DIR/app/view_helpers.sh"
fi
...
validate_sitemap() {
  ...
  if [[ "$url" == *.gz ]]; then
    content=$(echo "$content" | gunzip 2>> "${LOG_FILE:-/dev/null}")
    if [[ $? -ne 0 ]]; then
      return 1
    fi
  fi
  ...
  if echo "$content" | LC_ALL=C grep -q '<loc>' 2>> "${LOG_FILE:-/dev/null}"; then
    return 0
  elif echo "$content" | LC_ALL=C grep -q '<sitemap>' 2>> "${LOG_FILE:-/dev/null}"; then
    return 0
  elif echo "$content" | LC_ALL=C grep -q '<urlset' 2>> "${LOG_FILE:-/dev/null}"; then
    return 0
  elif echo "$content" | LC_ALL=C grep -q '<sitemapindex' 2>> "${LOG_FILE:-/dev/null}"; then
    return 0
  else
    return 1
  fi
...
validate_api_key() {
  ...
  case "$api_type" in
    weather|openweathermap)
      ...
      local response
      response=$(curl -s --connect-timeout "$timeout" "$test_url" 2>> "${LOG_FILE:-/dev/null}")
      if echo "$response" | grep -q '"cod":200' 2>> "${LOG_FILE:-/dev/null}"; then
        return 0
      elif echo "$response" | grep -q '"cod":"200"' 2>> "${LOG_FILE:-/dev/null}"; then
        return 0
      fi
      return 1
      ;;
...
validate_json_file() {
  ...
  if jq empty "$json_path" 2>> "${LOG_FILE:-/dev/null}"; then
    return 0
  else
    return 1
  fi

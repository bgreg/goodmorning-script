...
run_doctor() {
  ...
  if [[ -f "$config_file" ]]; then
    source "$config_file" 2>> "${LOG_FILE:-/dev/null}"
...
doctor_check_system_environment() {
  ...
  if [[ "$OSTYPE" == "darwin"* ]]; then
    local macos_version=$(sw_vers -productVersion 2>> "${LOG_FILE:-/dev/null}" || echo "unknown")
  ...
  # Timezone
  local tz="${TZ:-$(readlink /etc/localtime 2>> "${LOG_FILE:-/dev/null}" | sed 's|.*/zoneinfo/||')}"
  if [[ -n "$tz" ]]; then
  ...
  if [[ -d "$config_parent" ]]; then
    local available_kb=$(df -k "$config_parent" 2>> "${LOG_FILE:-/dev/null}" | awk 'NR==2 {print $4}')
    if [[ -n "$available_kb" ]]; then
      ...
      if [[ "$available_mb" -gt 100 ]]; then
        ...
        validation_fail "Very low disk space: ${available_mb}MB" "May affect logs"
      fi
    fi
  fi
...
doctor_check_network() {
  ...
  for endpoint in "${endpoints[@]}"; do
    if curl -s --connect-timeout 3 --max-time 5 -o /dev/null "https://$endpoint" 2>> "${LOG_FILE:-/dev/null}"; then
      (( reachable++ )) || true
    fi
  done
...
doctor_check_configured_paths() {
  ...
  if [[ -d "$logs_dir" ]]; then
    local log_count=$(find "$logs_dir" -name "*.log" -type f 2>> "${LOG_FILE:-/dev/null}" | wc -l | tr -d ' ')
  ...
  if [[ -d "$history_dir" ]]; then
    local history_count=$(find "$history_dir" -type f 2>> "${LOG_FILE:-/dev/null}" | wc -l | tr -d ' ')
  ...
  if [[ "${GOODMORNING_SHOW_GITHUB:-true}" == "true" ]]; then
    if command -v gh &>/dev/null; then
      if timeout 5 gh auth status &>/dev/null 2>&1; then
        local gh_user=$(gh api user --jq .login 2>> "${LOG_FILE:-/dev/null}" || echo "unknown")
...
doctor_check_config() {
  ...
  # Setup completion check (first priority)
  if [[ "${GOODMORNING_SETUP_COMPLETE:-}" == "true" ]]; then
    validation_pass "Setup has been completed"
  else
    validation_fail "Setup has not been run" "Run: ./setup.sh"
  fi
...
doctor_check_json_files() {
  ...
  local project_json_files=(
    "$script_dir/data/learning-sources.json"
    "$script_dir/data/sanity-maintenance-sources.json"
  )
...
doctor_check_urls() {
  ...
  if [[ ! -f "$learning_file" ]]; then
    learning_file="$script_dir/data/learning-sources.json"
  fi

  local sanity_file="$script_dir/data/sanity-maintenance-sources.json"

  # Check sitemaps from learning sources
  if [[ -f "$learning_file" ]] && command -v jq &>/dev/null; then
    local sitemap_count=$(jq '.sitemaps | length' "$learning_file" 2>> "${LOG_FILE:-/dev/null}" || echo "0")
    ...
    if [[ "$sitemap_count" -gt 0 ]]; then
      ...
      for ((i=0; i<sitemap_count; i++)); do
        local title=$(jq -r ".sitemaps[$i].title" "$learning_file" 2>> "${LOG_FILE:-/dev/null}")
        local url=$(jq -r ".sitemaps[$i].sitemap" "$learning_file" 2>> "${LOG_FILE:-/dev/null}")
    ...
    show_new_line
    local static_count=$(jq '.static | length' "$learning_file" 2>> "${LOG_FILE:-/dev/null}" || echo "0")
    ...
    if [[ "$static_count" -gt 0 ]]; then
      ...
      for ((i=0; i<static_count; i++)); do
        local title=$(jq -r ".static[$i].title" "$learning_file" 2>> "${LOG_FILE:-/dev/null}")
        local url=$(jq -r ".static[$i].url" "$learning_file" 2>> "${LOG_FILE:-/dev/null}")
        local short_title="${title:0:23}"
        [[ ${#title} -gt 23 ]] && short_title="${short_title}.."
        local short_url="${url:0:43}"
        [[ ${#url} -gt 43 ]] && short_url="${short_url}.."
  ...
  if [[ -f "$sanity_file" ]] && command -v jq &>/dev/null; then
    ...
    # Count total URLs
    local total_sanity=$(jq '[.categories[] | length] | add' "$sanity_file" 2>> "${LOG_FILE:-/dev/null}" || echo "0")
    echo_blue "  🤪 Sanity Maintenance Sources ($total_sanity total):"
    show_new_line

    # Get all categories
    local categories=$(jq -r '.categories | keys[]' "$sanity_file" 2>> "${LOG_FILE:-/dev/null}")

    printf "  %-25s %-45s %-6s %s\n" "Name" "URL" "Status" "Notes"
    echo_gray "  ─────────────────────── ───────────────────────────────────────────── ────── ─────────────────────"

    for category in ${(f)categories}; do
      local url_count=$(jq ".categories[\"$category\"] | length" "$sanity_file" 2>> "${LOG_FILE:-/dev/null}" || echo "0")

      for ((i=0; i<url_count; i++)); do
        local title=$(jq -r ".categories[\"$category\"][$i].title" "$sanity_file" 2>> "${LOG_FILE:-/dev/null}")
        local url=$(jq -r ".categories[\"$category\"][$i].url" "$sanity_file" 2>> "${LOG_FILE:-/dev/null}")
...
doctor_print_summary() {
  ...
  if [[ $VALIDATION_FAILED -eq 0 ]]; then
    if [[ $VALIDATION_WARNED -eq 0 ]]; then
      ...
      echo_yellow "  ⚠️  Checks passed with warnings. Some optional features may be limited."
    fi
  else

...
# Source this after colors.sh (provides show_new_line)
...
jq_extract() {
  local json="$1"
  local jq_path="$2"
  printf '%s' "$json" | jq -r "${jq_path} // empty" 2>> "$LOG_FILE"
}
...
fetch_url() {
  local url="$1"
  local timeout="${2:-10}"
  curl -s --max-time "$timeout" "$url" 2>> "$LOG_FILE"
}
...
fetch_url_compressed() {
  local url="$1"
  local timeout="${2:-10}"
  curl -s -L --compressed --max-time "$timeout" "$url" 2>> "$LOG_FILE"
}
...
# Print warning if previous command failed
# Usage: some_command; if_failed_say "Error message"
if_failed_say() {
  local exit_status=$?
  local message="$1"
  [ $exit_status -ne 0 ] && echo_yellow "  $message"
}

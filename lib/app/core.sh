...
# Check if already defined to prevent errors when script is sourced multiple times
(( ${+SPINNER_ITERATIONS_PER_SECOND} )) || typeset -gr SPINNER_ITERATIONS_PER_SECOND=10
(( ${+SPINNER_SLEEP_INTERVAL} )) || typeset -gr SPINNER_SLEEP_INTERVAL=0.1
(( ${+SPINNER_CLEAR_WIDTH} )) || typeset -gr SPINNER_CLEAR_WIDTH=70
...
_fetch_count_with_timeout() {
  ...
  # Run osascript once, capturing output directly
  count=$(timeout $timeout_seconds osascript "$script_path" 2>> "$LOG_FILE") || count=""

  # Validate result is a number, default to 0 if not
  [[ "$count" =~ ^[0-9]+$ ]] || count=0
...
fetch_with_spinner() {
  ...
  local output=$(cat "$temp_file" 2>> "${LOG_FILE:-/dev/null}")
  rm -f "$temp_file"
...
generate_iterm_image_sequence() {
  ...
  local file_size=$(wc -c < "$image_file" 2>> "$LOG_FILE" | tr -d ' ')
...
validate_image_file() {
  ...
  local file_type
  file_type=$(file -b "$image_file" 2>> "$LOG_FILE")
...
download_image() {
  ...
  while [[ $attempt -le $max_retries ]]; do
    curl -sL --max-time 15 "$url" -o "$output_file" 2>> "$LOG_FILE"

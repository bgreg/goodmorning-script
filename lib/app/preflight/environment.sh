...
# - Shell (zsh required)
# - Terminal type (iTerm2 recommended for inline photos and rich content)
...
check_permissions() {
  ...
  [[ -r "$script_dir/lib/app/core.sh" ]] || return 1

  return 0
}

###############################################################################
# check_setup_complete - Verify setup has been run
#
# Checks if GOODMORNING_SETUP_COMPLETE is set to "true", which indicates
# the user has run setup.sh at least once.
#
# Returns: 0 if setup complete, 1 otherwise
###############################################################################

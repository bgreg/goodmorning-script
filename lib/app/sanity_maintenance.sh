...
_fetch_xkcd_data() {
  local latest=$(fetch_url "https://xkcd.com/info.0.json" | jq -r '.num' 2>> "${LOG_FILE:-/dev/null}")
  ...
  if [ -n "$comic_data" ]; then
    local title=$(echo "$comic_data" | jq -r '.title' 2>> "${LOG_FILE:-/dev/null}")
...
_show_from_category() {
  ...
  local item_count=$(jq ".categories.${category} | length" "$json_file" 2>> "${LOG_FILE:-/dev/null}")
...
_show_random_from_categories() {
  ...
  local categories=(${(f)"$(jq -r "$jq_filter" "$json_file" 2>> "${LOG_FILE:-/dev/null}")"})
...
show_sanity_maintenance() {
  ...
  local comics_count=$(jq '.categories.comics | length' "$json_file" 2>> "${LOG_FILE:-/dev/null}")

  if [ -n "$comics_count" ] && [ "$comics_count" -gt 0 ]; then
    _show_from_category "$json_file" "comics"
    if_failed_say "No comics available"
    show_new_line

    _show_non_comic_resource "$json_file"
    if_failed_say "No other sources available"
  else
    # No comics, show one random item from any category
    _show_random_from_categories "$json_file" '.categories | keys[]'
    if_failed_say "No sources available"
  fi
  show_new_line
}